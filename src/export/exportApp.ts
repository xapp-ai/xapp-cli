 ...
 */
export async function exportApp(output: string, options?: { appId: string }): Promise<void> {
    const { appId } = options;
    ...
    const appPath = resolve(exportPath, `${app.appId}.json`);
    writeFileSync(appPath, JSON.stringify(app, undefined, 2));

    const intentsAndHandlersPath = resolve(exportPath, `handlers.json`);
    writeFileSync(intentsAndHandlersPath, JSON.stringify(intents, undefined, 2));
